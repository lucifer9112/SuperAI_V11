"""SuperAI V11 — backend/api/v1/chat.py"""
from fastapi import APIRouter, Depends
from fastapi.responses import StreamingResponse
from backend.app.dependencies import get_orchestrator, get_memory_service
...
async def chat(req: ChatRequest, orchestrator=Depends(get_orchestrator)) -> APIResponse:
    result = await orchestrator.chat(req)
    return APIResponse(data=result.model_dump())

@router.post("/stream", summary="Streaming chat (SSE)")
async def chat_stream(req: ChatRequest, orchestrator=Depends(get_orchestrator)) -> StreamingResponse:
    async def _gen():
...
async def get_history(session_id: str, limit: int = 20,
                      memory_svc=Depends(get_memory_service)) -> APIResponse:
