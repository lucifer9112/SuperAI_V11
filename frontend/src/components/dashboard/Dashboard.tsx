...
export function Dashboard() {
  ...
  return (
    <div className="flex h-full flex-col gap-4 overflow-y-auto p-4" style={{ background: "var(--bg)" }}>
      ...
      {status.gpu_info && (
        <div className="rounded-xl border p-3 text-xs" style={{ background: "var(--bg-card)", borderColor: "var(--border)" }}>
          ...
          <div className="flex flex-col gap-1">
            {status.models_loaded.map((model) => (
              <div key={model} className="flex items-center gap-2 text-xs">
                ...
                />
                <span className="truncate" style={{ color: "var(--text-muted)" }} title={model}>
                  {model}
                </span>
              </div>
            ))}
          ...
          <div className="grid grid-cols-2 gap-2 text-xs">
            ...
            <div className="rounded-lg p-2" style={{ background: "var(--bg-elevated)" }}>
              ...
              <p
                ...
                style={{ color: fbStats.avg_score >= 4 ? "#10b981" : fbStats.avg_score >= 3 ? "#f59e0b" : "#ef4444" }}
              >
...
function MetricCard({
  ...
  return (
    <div className="flex flex-col gap-2 rounded-xl border p-3" style={{ background: "var(--bg-card)", borderColor: "var(--border)" }}>
      ...
      {pct != null && (
        <div className="h-1 overflow-hidden rounded-full" style={{ background: "var(--bg-elevated)" }}>
          <motion.div
...
const ErrorState = ({ msg }: { msg: string }) => (
  <div
    className="m-4 rounded-xl border p-3 text-sm"
